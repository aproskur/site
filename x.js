...
const GameBoard = ({ size, theme }) => {
    ...
    const checkForMatch = () => {
        {
            ...
            if (numbersArray[firstIndex] === numbersArray[secondIndex]) {
                setMatchedPairs(prevMatchedPairs => [...prevMatchedPairs, numbersArray[firstIndex]]);
                updatePlayerDataForMatch(currentPlayerIndex);
                // In single-player mode, we don't need to consider changing turns
            } else {
                // Only proceed to change turns if in multiplayer mode
                if (numPlayers > 1) {
                    handleTurnAfterAction(false);
                }
            }
    ...
    //When a player finds a match,  to update their score or another relevant metric.
    const handleTurnAfterAction = (actionSuccess) => {
        // Check if the game is in multiplayer mode before deciding to change turns
        if (!actionSuccess && numPlayers > 1) {
            setCurrentPlayerIndex(prevIndex => (prevIndex + 1) % numPlayers);
        }
        // If actionSuccess is true or in single-player mode, there's no need to update currentPlayerIndex
    };

    const updatePlayerDataForMatch = (playerIndex) => {
        setPlayerData(currentPlayerData =>
            currentPlayerData.map((data, index) =>
                index === playerIndex ? { ...data, pairs: data.pairs + 1 } : data
            )
        );
    };
