...
const CategorySelection = ({ onCategorySelect }) => {
  const categories = ["Minecraft", "Movies", "TV-shows", "Countries", "Capital Cities", "Animals"];
...
function StartGamePopup() {
  ...
  //Manage visibility based on progress and guessed letters
  useEffect(() => {
    if (progress <= 0 || guessedLetters.size === 0) {
      setIsVisible(true);
    } else {
      setIsVisible(false);
    }
  }, [progress, guessedLetters.size]);

  // Starts the game with the selected category and hides the popup
  const handleStartGame = (category) => {
    startGame(category); // Pass the category name to the startGame function
    setIsVisible(false);
  };

  // Initiates category selection mode
  ...
  if (progress <= 0) {
    ...
    buttons = [
      { label: 'Start Game', onClick: initiateCategorySelection, color: 'rgb(var(--blue))' },
